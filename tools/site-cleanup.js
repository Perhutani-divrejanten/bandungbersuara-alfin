...
function removeFooterContact($) {
  $('div.container-fluid.bg-dark').each((_, el) => {
    ...
    // Remove only the Bandung Bersuara footer column and its social/contact content.
    block.find('div.col-lg-3.col-md-6.mb-5, div.col-lg-4.col-md-6.mb-4').each((_, col) => {
      const column = $(col);
      const heading = column.find('h5, h6').first().text().trim().toLowerCase();
      const html = column.html() || '';
      const hasBandungBersuara = /bandung\s+bersuara/i.test(heading) || /bandung\s+bersuara/i.test(html);
      const hasSocialLinks = /twitter\.com\/bandungbersuara|facebook\.com\/bandungbersuara|linkedin\.com\/company\/bandungbersuara|instagram\.com\/bandungbersuara|youtube\.com\/@bandungbersuara/i.test(html);
      const hasFollowTitle = column.find('h6').filter((_, h) => /ikuti kami/i.test($(h).text().trim().toLowerCase())).length > 0;

      if (hasBandungBersuara || hasSocialLinks || hasFollowTitle) {
        column.remove();
      }
    });
